...
public class RServe {
	public static void main(String[] args) {
		...
		try {
			...
			conn.eval("wordcount <- dbGetQuery(con, 'SELECT word, tfidf FROM wordcount WHERE tfidf > (SELECT AVG(tfidf) FROM wordcount) LIMIT 10')");
			conn.eval("palete<-brewer.pal(9,'Set1')");
			conn.eval("jpeg(filename = '/home/hadoop/rServeTmp/wordcloud4.jpeg',width = 400, height = 400)");
